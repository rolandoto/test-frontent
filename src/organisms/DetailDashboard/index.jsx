...
const DetailDasboard = (props) => {
  ...
  const totalId =
    jwt.result.id_hotel == 7 ||
    jwt.result.id_hotel == 3 ||
    jwt.result.id_hotel == 4 ||
    jwt.result.id_hotel == 23 ||
    jwt.result.id_hotel == 5 ||
    jwt.result.id_hotel == 6 ||
    jwt.result.id_hotel == 12
      ? true
      : false;

  const numbersRecepcion =
    (jwt.result.id_hotel == 13 && "573022395096") ||
    (jwt.result.id_hotel == 7 && "573022395096") ||
    (jwt.result.id_hotel == 23 && "573022395096") ||
    (jwt.result.id_hotel == 3 && "573007785193") ||
    (jwt.result.id_hotel == 4 && "573007785193") ||
    (jwt.result.id_hotel == 8 && "573007785193") ||
    (jwt.result.id_hotel == 5 && "573195550001") ||
    (jwt.result.id_hotel == 6 && "573195550001") ||
    (jwt.result.id_hotel == 12 && "573195550001");
  ...
  const findFirma =
    resultDashboard?.Estado == "3" ||
    resultDashboard?.Estado == "1" ||
    resultDashboard?.Estado == "5" ||
    resultDashboard?.Estado == "6";
  ...
  const [loadingFecha, setLoadingFecha] = useState({
    loading: false,
    error: false,
  });
  ...
  const [loadingTypeRoom, setLoadingTypeRoom] = useState({
    loading: false,
    error: false,
  });
  ...
  const n = moment(resultDashboard?.Fecha_nacimiento)
    .utc()
    .format("YYYY/MM/DD");
  ...
  const handChangeSave = () => {
    ServiceInfomeMovimiento({
      ...
      Fecha: now,
      Movimiento: `Se actualizo datos personales tipo habitacion ${resultFinish?.nombre} ${resultDashboard.Numero} nombre ${resultDashboard.Nombre} codigo reserva ${resultDashboard.id_persona}`,
  ...
  const handChecking = () => {
    if (!findFirma) {
      HttpClient.validCheckingAll({ ID: resultDashboard.ID_Habitaciones })
        .then((itemValid) => {
  ...
  const docu = tipoDocumento?.find(
    (index) => index?.ID == resultDashboard?.ID_Tipo_documento
  );

  const resultFinish = room?.find(
    (index) => index?.id_tipoHabitacion == resultDashboard?.ID_Tipo_habitaciones
  );
      ...
      HttpClient.insertPayABono({ data: inputPayValue })
        .then((index) => {
          ServiceInfomeMovimiento({
            Nombre_recepcion: jwt.result.name,
            Fecha: now,
            Movimiento: `Abono agregado tipo habitacion ${resultFinish?.nombre} ${resultDashboard.Numero} nombre ${resultDashboard.Nombre} codigo reserva ${resultDashboard.id_persona}`,
            id: jwt.result.id_hotel,
          })
            .then((index) => {
  ...
  const { handModalText } = UseModalText({
    handlModal: handClickInsertAbono,
    Text: "Agregar abono ?",
  });
  ...
  const total_Cobrar =
    resultDashboard?.valor_habitacion - resultDashboard?.valor_abono;
  const cobrar =
    parseInt(resultDashboard?.valor_habitacion) -
    parseInt(resultDashboard?.valor_abono);

  const totalPrice = DetailDashboard.reduce(
    (acum, item) =>
      acum +
      parseInt(item.valor_habitacion) -
      parseInt(item.valor_abono) -
      parseInt(inputPayValue.PayAbono),
    0
  );
  ...
  let dataOne = {
    ...
    Valor_habitacion:
      count == undefined ? resultDashboard?.valor_habitacion : count,
    Fecha_pago: now,
  };
  ...
  const hanAdd = () => {
    ...
    if (
      huespe.Tipo_documento == "" ||
      huespe.Num_documento == "" ||
      huespe.Nombre == "" ||
      huespe.Apellido == "" ||
      huespe.Celular == "" ||
      huespe.Correo == "" ||
      huespe.Fecha_nacimiento == "" ||
      huespe.Ciudad == "" ||
      huespe.Nacionalidad == ""
    ) {
      setError(true);
      ...
      ServiceAddHuespedes({
        ...
        .then((index) => {
          ...
          ServiceInfomeMovimiento({
            Nombre_recepcion: jwt.result.name,
            Fecha: now,
            Movimiento: `Se añadio huesped tipo habitacion ${resultFinish?.nombre} ${resultDashboard.Numero} nombre ${resultDashboard.Nombre} codigo reserva ${resultDashboard.id_persona}`,
            id: jwt.result.id_hotel,
          })
            .then((index) => {
  ...
  const handSubmit = () => {
    ServiceAddHuespedes({ id, huespe, data: dataCountPeople, dataPay: dataOne })
      .then((index) => {
        console.log(index);
        window.location.reload();
      })
    ...
    ServiceUpdateReservation({ id: resultDashboard.id_persona, data })
      .then((index) => {
        console.log(index);
    ...
    if (dataOne.Abono !== "COPNaN") {
      ServiceUpdateReservationpay({ id, dataOne })
        .then((index) => {
          console.log(index);
        })
        .catch((e) => {
  ...
  const hanDelete = () => {
    if (parseInt(resultDashboard.valor_abono) <= 0) {
      ServiDelteReservation({ id })
        .then((index) => {
          postDetailRoom({
            id: resultDashboard?.ID_Habitaciones,
            ID_estado_habitacion: 0,
          });
          ServiceInfomeMovimiento({
            Nombre_recepcion: jwt.result.name,
            Fecha: now,
            Movimiento: `Reserva eliminada tipo habitacion ${resultFinish?.nombre} ${resultDashboard.Numero} nombre ${resultDashboard.Nombre} codigo reserva ${id} `,
            id: jwt.result.id_hotel,
          })
            .then((index) => {
  ...
  const handClickPostTarifasReservation = async () => {
    if (isValidNumberOne(valorSolicitado) && isNaN(descripcion)) {
      await postInsertTarifas({
        ...
        Fecha: now,
        ID_reservation: id,
        name_reservation: resultDashboard.Nombre,
        codigo_reserva: `${resultDashboard?.Num_documento}${id}`,
        noches: day,
        Abono: parseInt(valor),
      });
  ...
  const hancPdf = () => {
    ServePdf({
      codigoReserva: documentByIdRoom,
      Nombre: resultDashboard?.Nombre,
      habitacion: `${resultFinish?.nombre}${resultDashboard.Numero}`,
      adults: resultDashboard?.Adultos,
      children: resultDashboard?.Ninos,
      tituloReserva: resultDashboard?.Nombre,
      abono: resultDashboard?.valor_abono,
      formaPago: resultDashboard?.forma_pago,
      telefono: resultDashboard.Celular,
      identificacion: resultDashboard?.Num_documento,
      correo: resultDashboard.Correo,
      urllogo: jwt?.result?.logo,
      tarifa: resultDashboard.valor_habitacion,
      ...
      .then((index) => {
        ...
        ServiceInfomeMovimiento({
          Nombre_recepcion: jwt.result.name,
          Fecha: now,
          Movimiento: `Descargar comprobante tipo habitacion ${resultFinish?.nombre} ${resultDashboard.Numero} nombre ${resultDashboard.Nombre},  codigo reserva ${resultDashboard.id_persona}`,
          id: jwt.result.id_hotel,
        })
          .then((index) => {})
          .catch((e) => {
  ...
  const hanClickAsear = () => {
    if (resultDashboard?.Estado != "3") {
      ServiceStatus({ id, ID_Tipo_Estados_Habitaciones: 5 })
        .then((index) => {
          Swal.fire({
            position: "center",
            icon: "success",
  ...
  const hanClickLimpia = async () => {
    if (resultDashboard?.Estado != "3") {
      await postDetailRoom({
        id: resultDashboard?.ID_Habitaciones,
        ID_estado_habitacion: 0,
      });
      ServiceStatus({ id, ID_Tipo_Estados_Habitaciones: 6 })
        .then((index) => {
          Swal.fire({
            position: "center",
            icon: "success",
  ...
  const handComprobante = UseModalText({
    handlModal: hancPdf,
    Text: "Descargar comprobante reserva?",
  });
  const hanclickEditar = UseModalText({
    handlModal: state ? handChangeSave : handChangeEdit,
    Text: "Editar la informacion de la reserva?",
  });
  const handleClickEliminar = UseModalText({
    handlModal: hanDelete,
    Text: "Estas seguro de eliminar la reserva ?",
  });

  const toPriceNigth = UsePrice({
    number: resultDashboard?.valor_dia_habitacion,
  });
  ...
  return (
    <>
      <div className="container-flex-init-global">
        <LoadingDetail
          titleLoading={"Datos actualizados"}
          loading={loading.loading}
          error={loading.error}
        />

        <LoadingDetail loading={true} titleLoading={"Detalle reserva"} />
        <LoadingDetail
        ...
        <div className="container-detail-dasboard-in-one">
          <div style={{ background: "#ebebeb" }} className="border-detail ">
            ...
            <span className="negrita-detail-reserva">
              {" "}
              {toPriceNigth.price}
            </span>
          </div>
          ...
          <div
            style={{
              background: `${totalPrice <= 0 ? "#17c964" : "#f21361"}`,
              color: "white",
            }}
          ...
          <div style={{ background: "#ebebeb" }} className="border-detail">
            ...
            <span className="negrita-detail-reserva">
              {resultFinish?.nombre} {resultDashboard.Numero}
            </span>
          </div>
          ...
          {resultDashboard.Foto_documento_adelante ? (
            <div
              style={{
                ...
                background:
                  "linear-gradient(32deg, rgba(34,193,195,1) 0%, rgba(143,102,189,1) 62%, rgba(253,187,45,1) 100%)",
                color: "white",
              }}
              className="border-detail"
            >
      ...
      <div className="container-flex-init-global">
        <div className="container-detail-dasboard-in">
          <input
            type="date"
            className="desde-detail"
            onChange={(e) => setspandOne(e.target.value)}
            defaultValue={fecha_inicio}
          />
          <input
            type="date"
            className="desde-detail"
            onChange={(e) => setspand(e.target.value)}
            defaultValue={fecha_final}
          />
          <h2 className="cod-reserva">
            <span className="title-code">COD:</span> X14A-
            {resultDashboard?.Num_documento}
            {id}
          </h2>
        </div>
      </div>
      ...
      <div className="init-photo top-one-detail-room">
        ...
        >
          <div className="container-detail-dasboard-in">
            ...
            <span className="desde-detail-two-title-photo-four">
              Doc frontal:
            </span>
            <span className="desde-detail-two-title-photo-four">
              Doc posterior:
          ...
          <div className="container-detail-dasboard-in-photo">
            ...
            <div>
              <Tooltip
                content={"Agregar pago sin coma, ni punto "}
                style={{ color: "white" }}
              >
                <button
                ...
                >
                  <span
                    className="negrita-detail-reserva  row-text-box"
                    style={{ marginLeft: "10px" }}
                  >
                    <CiCirclePlus fontSize={35} /> <span> Abono</span>
                  </span>
                </button>
              </Tooltip>
            </div>
            <div className="row-flex-one">
              <img
                src={`${
                  resultDashboard.Foto_documento_adelante
                    ? resultDashboard.Foto_documento_adelante
                    : "https://github.com/rolandoto/image-pms/blob/main/pdf_Mesa%20de%20trabajo%201_Mesa%20de%20trabajo%201%20(1).png?raw=true"
                }`}
                objectFit="initial"
                alt="Default Image"
                className={`img-photo`}
              ...
              <img
                src={`${
                  resultDashboard.Foto_documento_atras
                    ? resultDashboard.Foto_documento_atras
                    : "https://github.com/rolandoto/image-pms/blob/main/pdf_Mesa%20de%20trabajo%201_Mesa%20de%20trabajo%201%20(1).png?raw=true"
                }`}
                objectFit="initial"
          ...
          <div className="form-margintop-solicitud">
            <input
              ...
              type="text"
              className={`desde-detail-twophoto-two ${
            ...
            >
              <span
                ...
                style={{ marginLeft: "10px" }}
              >
                <CiCirclePlus fontSize={35} /> <span> Enviar</span>
              </span>
            </button>
          </div>
        </form>
      ...
      <div className="container-flex-init-one-center ">
        <div>
          <Button
            ...
            icon={
              <VscSymbolEvent
                fontSize={18}
                className="flex-contan"
                color="white"
              />
            }
          >
            {" "}
            <span className="text-words">Check in</span>{" "}
          </Button>
        </div>
        <div>
          <Button
            icon={
              <VscSignOut className="flex-contan" color="white" fontSize={18} />
            }
        ...
        <div
          className="name-pinter"
          onClick={handleClickEliminar.handModalText}
          data-tip
        ...
        >
          <div>
            <img
              width={33}
              src="https://medellin47.com/ico_pms/qcancel.svg"
              alt=""
            />
          </div>
        </div>
        ...
        <div
          className="name-pinter"
          onClick={hanclickEditar.handModalText}
          data-tip
          data-for="registerTip-1"
        >
          <div>
            <img
              width={33}
              src="https://medellin47.com/ico_pms/qedit.svg"
              alt=""
            />
          </div>
        </div>
        ...
        <div className="name-pinter" data-tip data-for="registerTip-2">
          <div onClick={handComprobante.handModalText}>
            <img
              width={33}
              src="https://medellin47.com/ico_pms/qdoc.svg"
              alt=""
            />
          </div>
        </div>
        <div className="container-checkbox">
          <input
            ...
            className={`checkbox-round  ${
              isChecked && "checkbox-round-click"
            } `}
        ...
        <div>
          ...
          >
            {" "}
            <span className="text-words">
              Total a cobro ${totaCobrar.toLocaleString()}{" "}
            </span>{" "}
          </Button>
        </div>
      </div>
      ...
      <div className="in-cehcki-out"></div>
            ...
            <div className="container-detail-dasboard-in in-type-button">
              <ul className="flex-contain">
                <li
                  className={`${
                    huesped
                      ? "desde-detail-three-estados-black-one-finish"
                      : "desde-detail-three-estados"
                  } `}
                  onClick={handHuesped}
                >
                  Huespedes: <PiUsersLight fontSize={25} /> {quyery?.length}{" "}
                </li>
                <li
                  className={`${
                    consumo
                      ? "desde-detail-three-estados-black"
                      : "desde-detail-three-estados"
                  } `}
                  onClick={handConsumo}
                >
                  Consumos: <PiShoppingBagOpenLight fontSize={25} />{" "}
                ...
                <li
                  ...
                  onClick={handPago}
                >
                  Pagos: <PiPaypalLogoLight fontSize={25} />{" "}
                  {product?.length > 0 ? product?.length : 0}{" "}
                </li>
                <li
                  className={`${
        ...
        <form className="container-flex-init init ono">
          ...
          {stateButton && (
            <div className="container-flex-init-one">
              <button
                className="button-dasboard-six-one-one-one"
                onClick={hanAdd}
                disabled={loadingHuesped}
              >
...
const Huesped = ({
  ...
  return (
    <Paper sx={{ width: "100%", margin: "10px" }}>
      ...
      >
        <LoadingDetail loading={true} titleLoading={"Huespedes"} />
          ...
          <TableBody>
            {DetailDashboard?.map((row, e) => (
              <TableRow key={e}>
                ...
                <TableCell
                  className="editar-checking"
                  onClick={handEditarReservas}
                >
                  <CiEdit fontSize={30} color="black" />
                </TableCell>
              </TableRow>
            ))}
            {query?.map((row, e) => (
              <TableRow key={e}>
                ...
                <TableCell
                  className="editar-checking"
                  onClick={() => handEditar(row.huespedes)}
                >
                  <CiEdit fontSize={30} color="black" />
                </TableCell>
              </TableRow>
      ...
      <div className="container-estado">
        <button
          className="button-checking-detail-checkout-estado"
          onClick={handChangeSubmit}
        >
...
const Consumo = (props) => {
  const { habitacion, setLoadingConsumo, loadinConsumo, product } = props;
  ...
  const handPayProduct = (id) => {
    if (state.forma_pago == null) {
      ...
      ServicePayReservationSore({ id, data })
        .then((index) => {
  ...
  return (
    <Paper sx={{ width: "90%", margin: "10px" }}>
      ...
      >
        <LoadingDetail loading={true} titleLoading={"Consumos"} />
        <Table>
          <TableHead>
            <TableRow>
              ...
              if (row.Pago_deuda == 0) {
                return (
                  <TableRow>
                    ...
                    <TableCell className="pay_deudado">
                      <span>Adeudado</span>
                    </TableCell>
                    ...
                    <TableCell>
                      {" "}
                      <span
                        className="pay_Pagado"
                        onClick={() => handPayProduct(row.ID)}
                      >
                        Pagar producto
                      </span>
                    </TableCell>
                ...
                return (
                  <TableRow>
                    ...
                    <TableCell>{row.Fecha} </TableCell>
                    <TableCell>Cop {row.Price} </TableCell>
                    <TableCell className="pay_Pagado-deuda">
                      <span>Pagado</span>
                    </TableCell>
                    <TableCell>
                      <span>Pagado</span>
...
const Pagos = (props) => {
  ...
  return (
    <Paper sx={{ width: "100%", margin: "10px" }}>
      ...
      >
        <LoadingDetail loading={true} titleLoading={"Pagos"} />
        <Table>
          <TableHead>
            <TableRow>
              <TableCell align="right">Fecha</TableCell>
...
const ItemCardPago = ({ index, typy_buy, setloading }) => {
  ...
  const totalDefinttion =
    index.Tipo_persona == "empresa" ? totalIva : totalDefinid;
  ...
  const handClickSave = () => {
    HttpClient.handchangeformapay({ ID: index.ID, Tipo_forma_pago: pay })
      .then((itemForma) => {
        setIsEditing(false);
        setloading((evenItem) => ({
          ...evenItem,
